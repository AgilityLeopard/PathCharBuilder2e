...
подходящие меч и щит против когтей и клыков? Или вам нужен мистический провидец, который черпает свои силы из запредельного для достижения своих целей? Почти все возможно</p>
		...
		<div class="content box-shadow text-center">
			<h3 class="ntm">Базово делаем</h3>
			<h4>Какой у него пол?</h4>
			<div class="radio-toolbar">
				<input v-model="gender" @change="selectGender" type="radio" name="gender" id="select-gender-male" value="male" checked>
				<label for="select-gender-male">Мужской</label>
				<input v-model="gender" @change="selectGender" type="radio" name="gender" id="select-gender-female" value="female">
				<label for="select-gender-female">Женский</label>
				<input v-model="gender" @change="selectGender" type="radio" name="gender" id="select-gender-other" value="other">
				<label for="select-gender-other">Другие</label>
			</div>
			<div v-if="gender == 'other'">
				<h4>Кто ваш перс?</h4>
				<input v-model="pronouns.they" @change="$emit('set-pronouns', pronouns)" type="text"  placeholder="они">
				<input v-model="pronouns.them" @change="$emit('set-pronouns', pronouns)" type="text"  placeholder="их">
				<input v-model="pronouns.their" @change="$emit('set-pronouns', pronouns)" type="text"  placeholder="заебали">
				<input v-model="pronouns.theirs" @change="$emit('set-pronouns', pronouns)" type="text"  placeholder="твиттерскими">
				<input v-model="pronouns.themself" @change="$emit('set-pronouns', pronouns)" type="text" placeholder="местоимениями">
			</div>
			<h4>Текущее имя?</h4>
